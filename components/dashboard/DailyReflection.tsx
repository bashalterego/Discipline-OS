...
export default function DailyReflection({ initialText }: DailyReflectionProps) {
    const { dailyLog, closeLog, isEditing, startEditLog, saveEditedLog } = useDashboardStore();
    const [text, setText] = useState(initialText || '');
    const [saving, setSaving] = useState(false);

    const handleBlur = async () => {
        if (isEditing) return; // Don't auto-sync while in full edit mode
        setSaving(true);
        try {
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            // Update the local text in the store version of dailyLog before saving
    ...
    return (
        <Card id="reflection" style={{ padding: '20px' }}>
            ...
            <button
                onClick={isEditing ? handleSave : (dailyLog?.log_closed ? startEditLog : closeLog)}
                disabled={saving}
